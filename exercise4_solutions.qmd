...
### Question 1
How many responders had both weekly rent and mortgage payments given? What are the potential reasons for this?
...
There were 105 responders with both weekly rent and mortgage payments. 

To understand why this is, we could first view this data (or a summary of the data) to see what other characteristics these responders share:
...
All responders in this group owned and lived in their own home. Most were leasehold properties, suggesting some of the weekly rent refers to lease payments. Other potential reasons could include shared ownership (which is not given as an option for tenure type), or responders that lived with renters in the same property.
...
One issue here is that the `rowwise` option replaces missing values with zeros. Some of the properties missing both rent and mortgage values may be a result of an incomplete survey.

To avoid this, we will use the first example (`if_else`) to create the newest variable.
