...
One of the major impacts on R programming as a result of the `tidyverse` approach is the use of **pipes**. A pipe symbol, either `magrittr`'s `%>%` or base R's `|>` can be read "and then". Pipes are able to combine multiple lines of coode into a single process. This makes code easier to read, reduces the number of objects required, and avoids duplicating objects.

For example, if we wanted to select the unique identifiers of respondents that were privately renting in the North East, we could do this in a single process:
...
::::{.stylebox-body}
Both the `|>` and `%>%` pipe work in similar ways. However, `|>` does not require additional packages and therefore reduces risk of errors in code. For the sake of consistency, it is recommended to use just the `|>` pipe in your code.

For information about the differences between these pipes, visit [this blog post](https://tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/). 

Pipes should be added at the end of a line, followed by a line break. The next line should be indented to indicate that it is part of the same process. To control indentation, visit `Global Options`.
::::
...
Pipes can be added using the keyboard shortcut *ctrl + shift + m* for Windows and *Command + shift + m* for Mac.

To control which pipe is added by this shortcut, visit the `Code` tab of the `Global Options` window.
:::
