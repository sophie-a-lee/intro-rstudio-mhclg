...
Graphs appear in the plot window in RStudio and can be opened in a new window using the ![zoom icon](img/zoom_shortcut.png) icon. Graphs in this window can also be copied and pasted into other documents using the ![export icon](img/export_shortcut.png) icon and selecting *Copy to clipboard*. 
...
```{r boxplot income tenure region}
ehs_tidy %>% 
